...
import pygame  # Import pygame for playing MIDI
import threading
...
class ChordApp:
    def __init__(self, master):
        ...
        self.mute_button.pack(side=tk.TOP, anchor='ne', padx=10, pady=10)
    ...
    def add_chord_row(self):
        ...
        for _ in range(4):
            ...
            # Bind selection event to update background color and play sound
            ...
            # Bind the indicator label to play the corresponding sound when clicked
            indicator_label.bind("<Button-1>", lambda event, cb=chord_combo: self.play_chord_sound(cb.get()))
    ...
    def check_chord_entry(self, combo, indicator_label):
        ...
        if not chord:  # If the input is empty
            ...
            self.play_chord_sound(chord)  # Play the MIDI file for the valid chord
        else:
            combo.state(['invalid'])
    ...
    def play_chord_sound(self, chord):
        """Play the MIDI file for the selected chord."""
        if self.is_muted:  # Don't play sound if muted
            return
        midi_path = os.path.join('chords_midi', f"{chord}.mid")
        if os.path.isfile(midi_path):
            self.play_midi(midi_path)  # Play the MIDI file if it exists
        else:
            print(f"Warning: MIDI file for chord '{chord}' not found.")
    ...
    def concatenate_midi(self):
        ...
        if chord_sequence:
            try:
                ...
                # self.play_midi(output_file)  # Play the MIDI file after concatenation
